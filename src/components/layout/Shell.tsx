  ...
  >
    {[0, 1].map((i) => (
      <motion.span
        ...
        style={{
          border: `1.5px solid ${active ? "#D6FF2F" : "rgba(214,255,47,0.55)"}`,
        }}
...
export function Shell({ children }: { children: React.ReactNode }) {
  ...
  const { data: cart } = useGetCart({
    query: { enabled: isLoggedIn, queryKey: ["/api/cart"] },
  });
  const cartCount =
          ...
          <div className="container mx-auto px-4 h-14 md:h-16 flex items-center justify-between">
            <Link href="/" className="flex items-center gap-2">
              ...
              <div className="leading-tight">
                <span className="text-lg md:text-xl font-bold tracking-tighter text-white block">
                  MotoHippi
                </span>
                <span className="text-[9px] md:text-[10px] text-primary tracking-widest uppercase hidden sm:block">
                  motohippi.com
                </span>
              </div>
            </Link>
            <div className="flex items-center gap-2 md:gap-4">
              <Link href="/login">
                <Button
                  variant="ghost"
                  size="sm"
                  className="text-white hidden sm:inline-flex h-9"
                >
                  Log in
                </Button>
              </Link>
              <Link href="/signup">
                <Button
                  size="sm"
                  className="bg-primary hover:bg-primary/90 text-black rounded-full px-4 md:px-6 h-9 md:h-10 text-sm font-bold"
                >
                  Join Free
                </Button>
              </Link>
        ...
        <div className="p-3 border-t border-white/5 space-y-0.5">
          <Link href="/cart">
            ...
            >
              ...
              {cartCount > 0 && (
                <span className="ml-auto w-5 h-5 rounded-full bg-primary text-black text-[10px] font-black flex items-center justify-center">
                  {cartCount}
                </span>
              )}
            </span>
        ...
        <div className="flex flex-col items-center p-2 border-t border-white/5 gap-1 pb-3">
          <Link href="/cart" title="Cart">
            ...
            >
              <ShoppingBag size={20} strokeWidth={1.8} />
              {cartCount > 0 && (
                <span className="absolute -top-0.5 -right-0.5 w-4 h-4 rounded-full bg-primary text-black text-[9px] font-black flex items-center justify-center">
                  {cartCount}
                </span>
              )}
            </span>
        ...
        <div className="mx-3 mb-2 bg-[#111]/90 backdrop-blur-2xl border border-white/10 rounded-2xl px-1 py-1 flex items-center justify-around shadow-2xl shadow-black/60">
          {MOBILE_MAIN.map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href}>
                <button
                  className={`relative flex flex-col items-center gap-0.5 px-3 py-2 rounded-xl min-w-[52px] min-h-[48px] justify-center transition-all ${
                    active
                      ? "text-primary"
